...
[Mesh]
  ...
  [bottom_punch_uncovered_external]
    type = BoundingBoxNodeSetGenerator
    bottom_left = '${fparse punch_radius * 39.0 / 40.0} ${fparse ram_cc_sinter_spacers_height + sinter_spacer_overhang_height} 0.0'
    top_right = '${fparse punch_radius * 81.0 / 80.0} ${fparse ram_cc_sinter_punch_height + (powder_height - die_wall_height) / 2.0} 0.0'
    input = inside_top_punch_secondary_subdomain
    new_boundary = 'bottom_punch_uncovered_right'
  []
  [top_punch_uncovered_external]
    type = BoundingBoxNodeSetGenerator
    bottom_left = '${fparse punch_radius * 39.0 / 40.0} ${fparse ram_cc_sinter_punch_height + (powder_height + die_wall_height) / 2.0} 0.0'
    top_right = '${fparse punch_radius * 81.0 / 80.0} ${fparse stack_with_powder + punch_height - sinter_spacer_overhang_height} 0.0'
    input = bottom_punch_uncovered_external
    new_boundary = 'top_punch_uncovered_right'
  []
...
[Functions]
  ...
  [bottom_ram_temperature]
    type = PiecewiseLinear
    data_file = bottom_ram_temperaturebc.csv
    format = columns
  []
  [top_ram_temperature]
    type = PiecewiseLinear
    data_file = top_ram_temperaturebc.csv
    format = columns
  []
[]
