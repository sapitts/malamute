...
sinter_spacer_overhang_radius = 0.0134375 ## is less than the 13.55 that actually exists but the modification is necessary to get the mesh to work
...
[Mesh]
  [bottom_sinter_spacer]
    ...
    dim = 2
    nx = 64 # 50 is n*2 from N1
    ny = 86 # 54 is n*2 from N1
  ...
  [bottom_sinter_overhang]
    type = GeneratedMeshGenerator
    dim = 2
    nx = 43 # 34 from the n*2 from N1
    ny = 6 #4 from the n*2 from N1
...
[Preconditioning]
  [vcp]
    type = VCP
    full = true
    lm_variable = 'bottom_sinter_punch_normal_lm bottom_punch_powder_normal_lm powder_top_punch_normal_lm top_punch_sinter_normal_lm inside_low_punch_normal_lm inside_powder_normal_lm inside_top_punch_normal_lm'
    primary_variable = 'disp_y disp_y disp_y disp_y disp_x disp_x disp_x'
    # coupled_variable = 'disp_x disp_y'
    preconditioner = 'LU'
    is_lm_coupling_diagonal = false  #only set to true if sure
...
[Executioner]
  ...
  petsc_options = '-pc_svd_monitor -ksp_converged_reason'  #-snes_converged_reason
